...
PhysicsMorph.prototype.drawNew = function() {
  var stage = this.parentThatIsA(StageMorph), scale = 1;
  if (stage) {
  ...
  var context = this.image.getContext('2d'), bodyAngle = this.physicsBody.angle,
      bodySin = Math.sin(bodyAngle), bodyCos = Math.cos(bodyAngle),
      bodyPos = this.physicsBody.position, aabb = this.physicsBody.getAABB(),
  ...
  this.physicsBody.shapes.forEach(function(shape) {
    if (shape.type == p2.Shape.BOX || shape.type == p2.Shape.CONVEX) {
      var v = shape.vertices, x = xOffset + bodyCos * shape.position[0] +
...
PhysicsMorph.prototype.physicsScale =
  ...
  var aabb = this.physicsBody.getAABB(), center = stage.center(),
      scale = stage.scale * this.physicsScale(),
      pos = new Point(
...
SpriteMorph.prototype.initPhysicsBlocks = function() {
  ...
  blocks.friction = {type: 'reporter', category: 'physics', spec: 'friction'};
...
SpriteMorph.prototype.getPhysicsContour = function() {
  ...
  var scale = 1.0 / this.physicsScale(), body = new p2.Body({
  ...
  if (this.costume) {
    ...
    body.addShape(new p2.Convex({
      vertices: [
        [1 * scale, 0 * scale], [-30 * scale, 8 * scale],
        [-30 * scale, -8 * scale]
      ]
    }));
  }
...
SpriteMorph.prototype.updateMorphicPosition = function() {
  ...
  var scale = this.physicsScale(), posX = this.physicsBody.position[0] * scale,
...
SpriteMorph.prototype.justDropped = function() {
  ...
  if (stage && world && world.hand && world.hand.phyPosTrace) {
    ...
    if (trace.length >= 2) {
      var i = trace.length - 1, n = Date.now();
...
HandMorph.prototype.processMouseMove =
  ...
  if (this.phyPosTrace instanceof Array) {
    ...
    this.phyPosTrace.push({x: event.screenX, y: event.screenY, t: Date.now()});
  }
}
...
SpriteIconMorph.prototype.userMenu = function() {
  var menu = this.phyUserMenu(), object = this.object;

  if (object instanceof SpriteMorph) {
    menu.addItem("debug", function() { object.debug(); });
  }
  return menu;
};
...
StageMorph.prototype.setPhysicsFloor = function(enable) {
  ...
  if (enable) {
    var ext = this.extent().multiplyBy(1.0 / this.physicsScale),
        body = new p2.Body({position: [0, 0], type: p2.Body.STATIC});
...
StageMorph.prototype.step = function() {
  ...
  if (this.physicsEngaged) {
    ...
    if (delta < 0.5) {
      var active = false, hats = this.allHatBlocksForSimulation();
...
PhysicsTabMorph.prototype.init = function(aSprite, sliderColor) {
  ...
  function toggleField(string, object, getter, setter, radio) {
    ...
    var getter2 = typeof getter === 'function' ? getter : function() {
      return this[getter];
    };
    var field = new ToggleMorph(
        radio ? 'radiobutton' : 'checkbox', object, setter, string, getter2);
  ...
  if (aSprite instanceof StageMorph) {
    ...
    elems.add(
        inputField(
            ...
            'pixel/m'));
    elems.add(toggleField("enable ground", aSprite, 'physicsFloor', function() {
    ...
    var radioDisabled = toggleField(
            ...
            function() { return !this.physicsMode; },
            function() {
              if (this.physicsMode) {
                this.physicsMode = '';
        ...
        radioStatic = toggleField(
            ...
            function() { return this.physicsMode === 'static' },
            function() {
        ...
        radioDynamic = toggleField(
            ...
            function() { return this.physicsMode === 'dynamic' },
            function() {
